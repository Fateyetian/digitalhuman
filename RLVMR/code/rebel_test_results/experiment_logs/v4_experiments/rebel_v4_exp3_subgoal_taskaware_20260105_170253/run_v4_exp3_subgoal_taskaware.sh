#!/bin/bash
# =============================================================================
# V4 实验 3: subgoal + task_aware (对照实验)
# =============================================================================
#
# 目的:
#   - 对照实验：在原有 subgoal 粒度基础上只启用 task_aware
#   - 用于验证 task_aware 的独立效果
#
# 改进点:
#   - belief_granularity: "subgoal" (保持原样)
#   - task_aware_grouping: true (新增)
#   - per_task_normalization: true (新增)
#
# 预期效果:
#   - 各任务类型性能更均衡
#   - 单样本组比例可能略有下降
...
EXPERIMENT_NAME="rebel_v4_exp3_subgoal_taskaware_$(date +%Y%m%d_%H%M%S)"
...
echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo "  V4 实验 3: subgoal + task_aware (对照实验)"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "目的: 验证 task_aware 的独立效果"
echo ""
echo "配置:"
echo "  - belief_granularity: subgoal (保持原样)"
echo "  - task_aware_grouping: true (新增)"
echo ""
echo "  - GPU: ${NUM_GPUS} × GPU"
echo "  - Epochs: ${EPOCHS}"

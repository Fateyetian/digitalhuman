#!/bin/bash
# =============================================================================
# ReBel V9 实验: 信念奖励衰减 (Belief Reward Decay with Cosine Annealing)
# =============================================================================
# 目的: 实现 Belief-Guided Curriculum Learning
# 配置: Warmup -> Peak -> Cosine Decay -> Maintain
#
# 衰减曲线 (缩小衰减周期，让结果奖励更早主导):
# 权重
# 1.0 |  ╭──╮
#     | ╱    ╲
#     |╱      ╲
...
# V9 信念奖励衰减参数 (缩小衰减周期)
DECAY_ENABLE=${DECAY_ENABLE:-true}
DECAY_METHOD=${DECAY_METHOD:-cosine}        # cosine / linear / exponential
WARMUP_EPOCHS=${WARMUP_EPOCHS:-3}           # warmup阶段: 0 -> 3 epoch
DECAY_START_EPOCH=${DECAY_START_EPOCH:-5}   # 开始衰减: 第5个epoch
DECAY_END_EPOCH=${DECAY_END_EPOCH:-30}      # 结束衰减: 第30个epoch
MIN_WEIGHT=${MIN_WEIGHT:-0.1}               # 最小权重: 0.1

export HF_HUB_OFFLINE=1
export TRANSFORMERS_OFFLINE=1
export VLLM_ATTENTION_BACKEND=XFORMERS

# 动态查找SFT模型路径 (排除.tar.gz文件，只查找目录)
find_sft_model() {
...
RESULTS_BASE="/fs-computility-new/UPDZ03_chengjun/huangsijie.p/rebel_results/v9_experiments"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

# 实验配置
EXP_NAME="belief_decay_${DECAY_METHOD}_w${WARMUP_EPOCHS}_s${DECAY_START_EPOCH}_e${DECAY_END_EPOCH}_m${MIN_WEIGHT}_seed${SEED}"
...
echo "═══════════════════════════════════════════════════════════════════"
echo "  ReBel V9: Belief-Guided Curriculum Learning"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "信念奖励衰减配置 (缩小周期版):"
echo "  - 启用衰减: ${DECAY_ENABLE}"
echo "  - 衰减方法: ${DECAY_METHOD}"
echo "  - Warmup阶段: 0 -> ${WARMUP_EPOCHS} epoch (权重: 0 -> 1)"
echo "  - Decay阶段: ${WARMUP_EPOCHS} -> ${DECAY_END_EPOCH} epoch (权重: 1 -> ${MIN_WEIGHT})"
echo "  - Maintain阶段: > ${DECAY_END_EPOCH} epoch (权重: ${MIN_WEIGHT})"
echo ""
echo "基本配置:"
echo "  - 实验名称: ${EXP_NAME}"
echo "  - 随机种子: ${SEED}"
...
python3 -m verl.trainer.main_ppo \
    ...
    +algorithm.rebel.belief_reward_decay.enable=${DECAY_ENABLE} \
    +algorithm.rebel.belief_reward_decay.method=${DECAY_METHOD} \
    +algorithm.rebel.belief_reward_decay.warmup_epochs=${WARMUP_EPOCHS} \
    ...
    +algorithm.rebel.belief_reward_decay.min_weight=${MIN_WEIGHT} \
    ...
    trainer.project_name='ReBel_V9_BeliefDecay' \
...
echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo "  V9 实验完成: Belief-Guided Curriculum Learning"
echo "═══════════════════════════════════════════════════════════════════"
echo ""

...
if [ ! -f "$DATA_DIR/train.parquet" ] || [ ! -f "$DATA_DIR/test.parquet" ]; then
    echo "❌ 错误: 本地数据不存在: $DATA_DIR"
    echo "需要的文件: train.parquet, test.parquet"
    exit 1
fi

echo "✅ 使用基础模型: $BASE_MODEL_PATH"
echo "✅ 使用本地数据: $DATA_DIR"
...
echo "[1/2] 跳过数据准备（使用已有本地数据）..."
echo "✅ 本地数据已存在: $DATA_DIR"
ls -lh $DATA_DIR/*.parquet

echo ""
echo "[2/2] 运行ReBel评测（基础模型）..."
echo "预计时间: 10-30分钟"
echo "请耐心等待..."
echo ""
...
# 使用分析脚本
if [ -f "analyze_rebel_results.py" ]; then
    python3 analyze_rebel_results.py ${OUTPUT_DIR} || echo "⚠️ 结果分析失败，但训练日志已保存"
else
    echo "⚠️ 分析脚本不存在，跳过自动分析"
fi
...
echo ""
echo "查看训练日志:"
echo "  cat ${OUTPUT_DIR}/training.log | less"
echo ""

...
\textit{Second}, structured belief prompting amplifies the step-level signal.
Combining GiGPO with the \texttt{<belief>} format lifts peak SR by a further
3.1 points (91.4\% $\to$ 94.5\%).
This stands in contrast to belief prompting without step-level mechanisms,
which yields only marginal and inconsistent gains (+1.6\% peak, $-$2.0\% late-average;
see~\S\ref{sec:ablation})---indicating that the structured output earns its keep
through richer grouping information, not through the prompt scaffold alone.

\textit{Third}, HiBO's hierarchical grouping stabilises late-stage training.
ReBel reaches 95.3\% peak and 89.1\% final SR, outperforming GiGPO + Belief
(94.5\% / 86.7\%).
The 0.8 pp peak difference understates HiBO's effect: its primary value is in
reducing the advantage estimate's variance, which shrinks the gap between peak
and sustained performance.
This is consistent with the late-average comparison: ReBel's Late-Avg SR (91.1\%)
exceeds GiGPO + Belief's (90.0\%) by a larger margin than the peak comparison.
...
Step-level advantage delivers the bulk of the gain (+9.4 pp); belief prompting
contributes +3.1 pp; HiBO with adaptive curriculum adds the final +0.8 pp at
the performance frontier---a regime where further gains are harder to obtain and
small absolute improvements carry disproportionate significance.
...
The comparison between ReBel Full and A1 (w/o HiBO) is instructive.
The 0.8 pp peak gap is modest, yet A1's final SR (91.4\%) \emph{exceeds}
ReBel Full's (89.1\%).
This inversion is consistent with reward shaping dynamics: HiBO's denser advantage
signal enables the policy to reach higher performance but also increases exploration
late in training, widening the peak-to-final gap.
The benefit of HiBO is better read from the learning curve than from the final-epoch
snapshot (Figure~\ref{fig:training_curves}).

Removing the belief reward (A3) drops peak SR by 3.1 pp, confirming that the
intrinsic signal retains value even as its curriculum weight diminishes.
A4 (w/o adaptive decay) matches A3's peak at 92.2\%---two components each
responsible for comparable absolute gains---and its 90-epoch ceiling makes the
final/late-avg comparison conservative.
...
\begin{table}[t]
  \centering
  \caption{%
    \textbf{WebShop generalization results} (ReBel Full, three seeds).
    GRPO and GiGPO baselines on WebShop are currently running;
    results will be incorporated in the final version.
...
These results demonstrate that ReBel's three components---HiBO grouping, belief
reward curriculum, and structured prompting---transfer to a different task family
with only environment-specific prompt adjustments.
Baseline comparisons (GRPO, GiGPO on WebShop) are in progress; preliminary
monitoring shows both methods struggling with the early-episode dependency
structure, consistent with the ALFWorld findings.
